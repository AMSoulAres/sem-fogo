...
import type { Camera, ZoomStream } from '~/types'
...
const { groups, createGroup, zoomStreams } = useCameraData()

const isOnline = computed(() => props.camera.status === 'online')

// Algumas câmeras tem nome de região repetida, então usei o id para diferenciar: 'physical_camera_7' → 'Câm. 7'
const physCamLabel = computed(() => {
  const match = props.camera.physicalCameraName?.match(/\d+/)
  return match ? `Cam. ${match[0]}` : props.camera.physicalCameraName
})

// ZoomStreams ativos nesta câmera (CaptureProcess no backend)
// O processo base (1x1 q0) é ignorado, focando apenas em recortes/zooms.
const zoomStreamsForCard = computed(() =>
  zoomStreams.value.filter((s: ZoomStream) =>
    s.cameraId === props.camera.id && !(s.quadrantZoom.n_lines === 1 && s.quadrantZoom.n_cols === 1 && s.quadrantZoom.quad_number === 0)
  )
)
...
const headerColorClasses = computed(() => {
  if (riskTier.value === 'critical') return 'border-b border-red-700/60 bg-red-50 dark:bg-red-900/80'
  if (riskTier.value === 'high' || riskTier.value === 'medium') return 'border-b border-orange-500/20 dark:border-orange-600/30 bg-orange-50 dark:bg-orange-900/12'
...
<template>
  ...
  >
    <div class="rounded-lg overflow-hidden">
      <UCard :ui="{ body: 'p-0 sm:p-0', header: headerClass }">
        <template #header>
          ...
          <div class="flex items-center gap-2 overflow-hidden min-w-0">
              ...
              <div class="min-w-0">
                ...
                <p v-if="!compact" class="text-[10px] text-gray-400 dark:text-gray-500 truncate leading-tight">
                  {{ physCamLabel }}
                </p>
              </div>
            </div>
        ...
        >
          <!-- Imagem do backend via proxy ou placeholder -->
          <img
          ...
          >
            <UIcon name="i-heroicons-video-camera" class="w-12 h-12 text-gray-600" />
          </div>

          <!-- Indicador de zoom ativo nesta câmera (CaptureProcess ativo) -->
          <div v-if="zoomStreamsForCard.length > 0" class="absolute top-2 right-2 flex gap-1 z-10">
            <UBadge color="primary" variant="solid" size="xs" class="shadow-lg flex gap-1 items-center font-bold">
          ...
          <div
            v-if="prob > 0"
            class="absolute bottom-0 left-0 right-0 bg-linear-to-t p-3 flex items-end justify-between"
          ...
          >
            ...
            <div class="flex flex-col items-center gap-1">
              <template v-if="riskTier === 'critical'">
                ...
                <span v-if="!compact" class="text-[9px] text-red-400 font-bold uppercase tracking-widest animate-pulse">Alerta!</span>
              </template>
